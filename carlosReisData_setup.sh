...
(crontab -l 2>&1; echo "3 20 */1 * * cd $PWD/carlosReis/bin && /usr/bin/python3 $PWD/carlosReis/bin/crypto_join_copy.py $PWD") | crontab -
# Crawler do dólar uma vez por dia às 18:00
(crontab -l 2>&1; echo "2 20 */1 * * cd $PWD/carlosReis/bin && /usr/bin/python3 $PWD/carlosReis/bin/dolar_crawler.py $PWD") | crontab -
...
(crontab -l 2>&1; echo "4 20 */1 * * cd $PWD/carlosReis/bin && $PWD/carlosReis/bin/carlosReisData_send_to_hdfs.sh $PWD") | crontab -
